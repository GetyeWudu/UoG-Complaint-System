...
function SuperAdminDashboard() {
  ...
  const fetchDashboardData = async () => {
    try {
      const res = await api.get('complaints/dashboards/super-admin/stats/');
      setStats(res.data);
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-100">
      <header className="bg-blue-600 text-white shadow-lg">
        <div className="container mx-auto px-4 py-4 flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold">{t('dashboard.superAdmin.systemStats')}</h1>
      ...
      <div className="container mx-auto px-4 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          ...
          <div className="bg-white p-6 rounded-lg shadow">
            <div className="text-gray-500 text-sm font-medium">{t('dashboard.superAdmin.userActivity')}</div>
            <div className="text-3xl font-bold text-blue-600 mt-2">{stats?.total_users || 0}</div>
          </div>
          <div className="bg-white p-6 rounded-lg shadow">
        ...
        {stats?.users_by_role && stats.users_by_role.length > 0 && (
          <div className="bg-white rounded-lg shadow p-6 mb-6">
            <h2 className="text-xl font-bold mb-4">{t('dashboard.superAdmin.userActivity')}</h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {stats.users_by_role.map((role, idx) => (
